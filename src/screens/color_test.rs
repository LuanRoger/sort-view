...
use eframe::egui::{
    Align2, Color32, Context, FontId, Image, Label, Mesh, Painter, Pos2, Rect, Response, Rgba,
    RichText, Sense, Shape, Stroke, StrokeKind, TextureHandle, TextureOptions, Ui, Vec2,
    emath::GuiRounding as _, epaint, lerp, pos2, vec2, widgets::color_picker::show_color,
};
...
impl TextureManager {
    fn get(&mut self, ctx: &Context, gradient: &Gradient) -> &TextureHandle {
...
fn paint_fine_lines_and_text(painter: &Painter, mut rect: Rect, color: Color32) {
    {
        let mut y = 0.0;
...
fn pixel_test_lines(ui: &mut Ui) {
    ...
    for x in 0..n / 2 {
        ...
        painter.rect_filled(rect_px / pixels_per_point, 0.0, Color32::WHITE);
        let rect_px = rect_px.translate(vec2(1.0, 0.0));
        painter.rect_filled(rect_px / pixels_per_point, 0.0, Color32::BLACK);
    }
...
fn text_on_bg(ui: &mut Ui, fg: Color32, bg: Color32) {
    assert!(
        fg.is_opaque(),
